...
void simulateGame(int** map, int height, int width) {
    ...
    for (x = 1; x < width - 1; x++) {
        for (y = 1; y < height - 1; y++) {
            liveCells = map[y - 1][x - 1] + map[y - 1][x] + map[y - 1][x + 1] + map[y][x - 1] + map[y][x + 1] + map[y + 1][x - 1] + map[y + 1][x] + map[y + 1][x + 1];
...
void readBitmapHeader(FILE* file, BitmapHeader* bmp) {
    ...
    bmp->width = bmp->header[21] * 256 * 256 * 256 + bmp->header[20] * 256 * 256 + bmp->header[19] * 256 + bmp->header[18];
    bmp->height = bmp->header[25] * 256 * 256 * 256 + bmp->header[24] * 256 * 256 + bmp->header[23] * 256 + bmp->header[22];
